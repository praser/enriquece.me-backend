...
Funcionalidade: Listar todas as contas do usuário
  ...
  Cenário: Quando um usuário autenticado solicita a lista das suas contas
    ...
    Quando o backend receber uma requisição autenticada para "http://api.example.com/accounts" através do método "GET"
  ...
  Cenário: Quando um usuário não autenticado solicita a lista de contas
    Quando o backend receber uma requisição não autenticada para "http://api.example.com/accounts" através do método "GET"
