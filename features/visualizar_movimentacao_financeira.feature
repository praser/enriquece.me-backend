...
@movimentacao_financeira @visualizar_movimentacao_financeira

Funcionalidade: Visualizar movimentação financeira
  Para gerenciar minhas movimentações financeiras
  Como um usuário comum
  Eu quero poder visualizar os dados de uma determinada movimentação financeira
  ...
  Contexto:
    ...
    | Feira     | 21.95 | 2017-07-01 | true | Conta corrente principal | Alimentação | Supermercado | johndoe@exemplo.com  |
  ...
  Cenário: Quando um usuário autenticado solicita acesso aos dados de uma de suas transações financeiras
    Dado que o usuário está autenticado no sistema através do email "johndoe@exemplo.com" e da senha "123456"
    Quando o backend receber uma requisição autenticada para exibir dados da transação financeira "Feira"
    ...
    E o campo "descrição" da transação financeira deve ser "Feira"
    E o campo "valor" da transação financeira deve ser "21.95"
    E o campo "data" da transação financeira deve ser "2017-07-01"
  
  Cenário: Quando um usuário autenticado solicita acesso aos dados de uma transação financeira de outro usuário
    Dado que o usuário está autenticado no sistema através do email "johndoe@exemplo.com" e da senha "123456"
    Quando o backend receber uma requisição autenticada para exibir dados da transação financeira "Gasolina"
  ...
  Cenário: Quando um usuário não autenticado solicita acesso aos dados de uma transação financeira
    Quando o backend receber uma requisição não autenticada para exibir dados da transação financeira "Gasolina"
    Então a resposta deve possuir status "401"
    E a resposta deve possuir o content type "application/json; charset=utf-8"
    E o corpo da resposta deve conter uma mensagem informando que o acesso foi negado